...
def _status_label(status: str) -> str:
    return _STATUS_LABELS.get(status, status or "Record created")


class ReloadCasinoOfferTemplateDialog(QDialog):
    def __init__(self, data_dir: Path, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Reload Casino Offer Templates")
        self.resize(920, 600)
        self.db = AppDatabase(data_dir)
        self.templates = self.db.fetch_reload_casino_offer_templates()
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Reload Casino Offer Templates", self)
    ...
    def accept(self) -> None:
        ...
        self.db.replace_reload_casino_offer_templates(normalized)
        self.db.refresh_reload_casino_offer_instances(horizon_start, horizon_end)
        super().accept()


class ReloadCasinoOffersPanel(ReloadOffersPanelBase):
    def _panel_title(self) -> str:
        return "Reload Casino Offers"

    def _table_headers(self) -> list[str]:
        return ["Status", "Bookie", "Promo", "Deposit", "Notes"]

    def _table_column_widths(self) -> tuple[int, ...]:
        return (130, 150, 240, 100)

    def _amount_columns(self) -> set[int]:
        return {3}

    def _empty_state_text(self) -> str:
        return "No reload casino offers for this date."
    ...
    def _create_template_dialog(self, parent: QWidget) -> QDialog:
        return ReloadCasinoOfferTemplateDialog(self.db.path.parent, parent)

    def _refresh_instances_window(self, window_start: str, window_end: str) -> None:
        self.db.refresh_reload_casino_offer_instances(window_start, window_end)

    def _fetch_instances_for_date(self, date_iso: str) -> list[dict[str, str]]:
        return self.db.fetch_reload_casino_offer_instances_for_date(date_iso)

    def _link_instance_record(self, instance_id: str, record_id: str) -> None:
        self.db.set_reload_casino_offer_instance_casino_record(instance_id, record_id)
...
__all__ = ["ReloadCasinoOffersPanel", "ReloadCasinoOfferTemplateDialog"]
